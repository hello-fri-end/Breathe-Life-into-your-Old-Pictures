import cv2
import numpy as np
from ImproveLighting import guidedfilter
...
def dehaze(I, tmin, w, alpha, omega, p, eps, reduce=False):
    ...
    refined_t = guidedfilter.guided_filter(normI, corrected_t, w, eps)
...
def enhance(image):
    ...
    I = np.asarray(image, dtype=np.float64) # Convert the input to an array.
    I = I[:, :, :3] / 255
